...
boolean AS5040::begin (byte mode, boolean reverse, unsigned int offset)
{
  ...
  if (_pinPROG == 0xFF)
  {
	...
	//modificar todos os delayMicroseconds(1) para delayMicroseconds(2) daqui pra baixo
	// para todos o tempo de subida e descida do CLK sejam 2 microsegundos
  ...
  // comentar isso aqui pra gravar---> deixar pinProg=LOW ---> CSnpin = HIGH ----> CLK = LOW
  // digitalWrite (_pinCS, LOW) ;
  // delayMicroseconds (1) ;
...
int AS5040::Zaxis ()
{
  ...
  return 3 ; // invalid case, must return something harmless
}
